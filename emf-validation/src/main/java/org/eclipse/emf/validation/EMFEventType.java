 ...
 * types. The first type correspond to the <code>int</code> constants defined by
...
public final class EMFEventType implements Serializable
{
     ...
     */
    public static final EMFEventType ADD = new EMFEventType("Add", //$NON-NLS-1$
                                                            true, Notification.ADD);

    /**
     * The EMF "Add Many" event (corresponds to {@link Notification#ADD_MANY}).
     */
    public static final EMFEventType ADD_MANY = new EMFEventType("Add Many", //$NON-NLS-1$
                                                                 true, Notification.ADD_MANY);

    /**
     * The EMF "Move" event (corresponds to {@link Notification#MOVE}).
     */
    public static final EMFEventType MOVE = new EMFEventType("Move", //$NON-NLS-1$
                                                             true, Notification.MOVE);

    /**
     * The EMF "Remove" event (corresponds to {@link Notification#REMOVE}).
     */
    public static final EMFEventType REMOVE = new EMFEventType("Remove", //$NON-NLS-1$
                                                               true, Notification.REMOVE);

    /**
     * The EMF "Remove Many" event (corresponds to
     * {@link Notification#REMOVE_MANY}).
     */
    public static final EMFEventType REMOVE_MANY = new EMFEventType("Remove Many", //$NON-NLS-1$
                                                                    true, Notification.REMOVE_MANY);

    /**
     * The EMF "Set" event (corresponds to {@link Notification#SET}).
     */
    public static final EMFEventType SET = new EMFEventType("Set", //$NON-NLS-1$
                                                            true, Notification.SET);

    /**
     * The EMF "Unset" event (corresponds to {@link Notification#UNSET}).
     */
    public static final EMFEventType UNSET = new EMFEventType("Unset", //$NON-NLS-1$
                                                              true, Notification.UNSET);

    /**
     * The EMF "Resolve" event (corresponds to {@link Notification#RESOLVE}).
     */
    public static final EMFEventType RESOLVE = new EMFEventType("Resolve", //$NON-NLS-1$
                                                                true, Notification.RESOLVE);

    /**
     * The EMF "Removing Adapter" event (corresponds to
     * {@link Notification#REMOVING_ADAPTER}).
     */
    public static final EMFEventType REMOVING_ADAPTER = new EMFEventType("Removing Adapter", //$NON-NLS-1$
                                                                         false,
                                                                         Notification.REMOVING_ADAPTER);

    /**
     * A custom "Create" event, which is not implemented by EMF any longer but
     * may be simulated by clients of the validation framework. (Corresponds to
     * <code>0</code>).
     */
    public static final EMFEventType CREATE = new EMFEventType("Create", //$NON-NLS-1$
                                                               true, 0);  // use
                                                                          // the
                                                                          // literal
                                                                          // value
     ...
     * Object</i> pattern. It indicates the absence of an EMF event trigger.
     */
    public static final EMFEventType NULL = new EMFEventType("none", //$NON-NLS-1$
                                                             false, -1);

    /** All of my values. */
    private static final List<EMFEventType> predefinedInstances =
            Collections.unmodifiableList(Arrays.asList(new EMFEventType[]{ADD, ADD_MANY, MOVE,
                                                                          REMOVE, REMOVE_MANY, SET,
                                                                          UNSET, RESOLVE,
                                                                          REMOVING_ADAPTER, CREATE,
                                                                          NULL,}));

    private static final List<EMFEventType> instances =
            new ArrayList<EMFEventType>(predefinedInstances);
    ...
    private EMFEventType(String name, boolean featureSpecific, int notificationTypeCode)
    {
        ...
        if (nameToInstance.containsKey(name)) {
            IllegalArgumentException e =
                    new IllegalArgumentException("Duplicate event type name: " + name); //$NON-NLS-1$
        ...
        nextNotificationTypeCode = (notificationTypeCode + 1 > nextNotificationTypeCode)
    ...
    /**
     * Obtains all predefined values of the enumeration, that is, those who were
     * not contributed through the extension point XML
     * @since 1.1
    ...
    /**
     * Queries whether I am the <i>Null Object</i> of this enumeration. In
     * general, <code>null</code> pointers are never used with this type.
