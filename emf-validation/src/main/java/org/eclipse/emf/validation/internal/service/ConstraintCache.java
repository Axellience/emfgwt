...
public class ConstraintCache implements IModelConstraintProvider
{
    ...
    /**
     * Mapping of {@link EClass} ==&gt; {@link EClassBucket}. The map is a weak
     * map to avoid interfering with garbage-collection of EMF metamodels (and
     * to clean up the cache when metamodels disappear!).
     */
    private final Map<EClass, EClassBucket> buckets = new java.util.HashMap<EClass, EClassBucket>();
    ...
    private static class EClassBucket
    {
        ...
        private final Map<EMFEventType, Map<String, Collection<IModelConstraint>>> liveConstraints =
                new java.util.HashMap<EMFEventType, Map<String, Collection<IModelConstraint>>>();
        ...
        if (notification.getNotifier() instanceof EObject) {
            ...
            if (cached == null) {
                if (Trace.shouldTrace(EMFModelValidationDebugOptions.CACHE)) {
                    Trace.trace(EMFModelValidationDebugOptions.CACHE,
                                "Cache missed live constraints for: " //$NON-NLS-1$
                                                                      + Trace.toString(new Object[]{qualifiedName(eObject.eClass()),
                                                                                                    eventType,
                                                                                                    featureName}));
                }
                                                            ...
                                                            Collection<IModelConstraint> constraints)
    {
        ...
        if (cached == null) {
            ...
            // not cached, yet? Ask my providers
            GetBatchConstraintsOperation operation = new GetBatchConstraintsOperation(false);  // must
                                                                                               // use
                                                                                               // false
                                                                                               // to
                                                                                               // cache
    ...
    public void replace(IModelConstraint oldConstraint, IModelConstraint newConstraint)
    {
        if (Trace.shouldTrace(EMFModelValidationDebugOptions.CACHE)) {
            Trace.trace(EMFModelValidationDebugOptions.CACHE,
                        "Cache replacing: " //$NON-NLS-1$
                                                              + oldConstraint + " with: " //$NON-NLS-1$
                                                              + newConstraint);
        }
