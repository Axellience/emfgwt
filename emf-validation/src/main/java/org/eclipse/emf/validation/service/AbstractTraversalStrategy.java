...
public abstract class AbstractTraversalStrategy implements ITraversalStrategy
{
    ...
    /**
     * Factor by which to scale progress monitoring steps in order that
     * recursion through sub-progrss-monitors results in visible progress (i.e.,
     * the progress doesn't go to 100% in one step) and a decent response time
     * to a user cancel request.
     */
                               ...
                               IProgressMonitor progressMonitor)
    {
        ...
        if (scaledSize == taskSize) {
            ...
            monitor = new SubProgressMonitor(progressMonitor, scaledSize, // covers
                                                                          // the
                                                                          // whole
                                                                          // progress
            ...
            monitor.beginTask("", //$NON-NLS-1$
                              taskSize);  // recursion counts one element at a
                                          // time
        }
    ...
    /**
     * Implements the interface method by advancing the progress monitor by a
     * single work unit.
     */
    @Override
