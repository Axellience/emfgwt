 ...
 */
public abstract class AbstractConstraintProvider implements IModelConstraintProvider// ,
                                                                                    // IExecutableExtension
{

    private String[]                     namespaceUris;
    private final List<IModelConstraint> myConstraints =
    ...
    private class ConstraintProxy implements IModelConstraint
    {
        ...
        /**
         * Lazily initializes my delegate constraint before invoking it, if
         * necessary.
         */
        @Override
        public IStatus validate(IValidationContext ctx)
        {
            ...
            try {
                if (Trace.shouldTrace(EMFModelValidationDebugOptions.CONSTRAINTS_EVALUATION)) {
                    Trace.trace(EMFModelValidationDebugOptions.CONSTRAINTS_EVALUATION,
                                "Delegating validate() method to: " //$NON-NLS-1$
                                                                                       + delegate
                                                                                       + " for: " //$NON-NLS-1$
                                                                                       + descriptor);
                }
            ...
            for (ListIterator<IModelConstraint> iter =
                    getConstraints().listIterator(); iter.hasNext();)
            {
     ...
     * default implementation delegates to the {@link ConstraintFactory} to
     * create the constraint, if the specified <tt>descriptor</tt> is of a known
     * type and a registered {@link IConstraintParser} is available to parse the
     * descriptor. If either of these conditions does not hold, then a disabled
     * constraint implementation will be returned (one that always returns an
     * info status indicating that it is disabled).
     * <p>
     ...
     * constraints lazily.
     * @param descriptor the descriptor of the constraint for which a proxy is
     *        to be created
    ...
    /**
     * {@inheritDoc}
     * @throws CoreException if the subclass implementation of this method
     *         throws on an error in accessing the <code>config</code> or for
