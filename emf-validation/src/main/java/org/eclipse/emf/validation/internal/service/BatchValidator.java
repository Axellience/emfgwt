/******************************************************************************
 * Copyright (c) 2003, 2010 IBM Corporation, Zeligsoft Inc., and others. All
 * rights reserved. This program and the accompanying materials are made
 * available under the terms of the Eclipse Public License v1.0 which
 * accompanies this distribution, and is available at
 * http://www.eclipse.org/legal/epl-v10.html Contributors: IBM Corporation -
 * initial API and implementation Zeligsoft - Bug 218765 $Id$
 ****************************************************************************/
...
/**
 * Basic implementation of the {@link IBatchValidator} interface. Ensures that,
 * in cases of multiple selection where recursion is desired, validation is not
 * repeated on selected elements that are contained in other selected elements.
...
public class BatchValidator extends AbstractValidator<EObject> implements IBatchValidator
{
    ...
    private ITraversalStrategy defaultTraversalStrategy = new DefaultRecursiveTraversalStrategy();
                                             ...
                                             Set<IClientContext> clientContexts)
    {
        ...
        List<IStatus> result = new java.util.ArrayList<IStatus>(64);  // anticipate
                                                                      // large
                                                                      // scale
     ...
     */
    private void validate(ITraversalStrategy traversal, List<IStatus> evaluationResults,
                          AbstractValidationContext ctx, Collection<? extends EObject> objects,
                          GetBatchConstraintsOperation operation,
                          Set<IClientContext> clientContexts)
    {
            ...
            while (traversal.hasNext()) {
                ...
                if (recomputeClients) {
                    Collection<IClientContext> contexts =
                            ClientContextManager.getInstance().getClientContextsFor(next);
                    ctx.setClientContexts(contexts);
                    clientContexts.addAll(contexts);
                }

                traversal.elementValidated(next, validate(ctx, next, operation, evaluationResults));
     ...
     */
    private IStatus validate(AbstractValidationContext ctx, EObject eObject,
                             GetBatchConstraintsOperation operation, List<IStatus> results)
    {
        if (Trace.shouldTraceEntering(EMFModelValidationDebugOptions.PROVIDERS)) {
            Trace.entering(getClass(),
                           "validate", //$NON-NLS-1$
                           new Object[]{eObject});
        }
     ...
     * set. These will cue the {@link MarkerUtil} to clear markers from
     * resources that had no problems.
    ...
    private static class DefaultRecursiveTraversalStrategy implements ITraversalStrategy
    {
                                   ...
                                   IProgressMonitor monitor)
        {
            ...
            for (Map.Entry<ITraversalStrategy, Collection<EObject>> next : delegates.entrySet()) {

                SubProgressMonitor sub =
                        new SubProgressMonitor(monitor, 1024,
        ...
        private Map<ITraversalStrategy, Collection<EObject>> initDelegates(Collection<? extends EObject> traversalRoots)
        {
            ...
            for (EObject next : traversalRoots) {
                ITraversalStrategy delegate =
                        TraversalStrategyManager.getInstance().getTraversalStrategy(next);
