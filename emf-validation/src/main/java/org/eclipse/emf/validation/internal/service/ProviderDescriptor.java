 ...
 * IBM Corporation - initial API and implementation Radek Dvorak (Borland) -
 * Bugzilla 165661
 ****************************************************************************/
...
public class ProviderDescriptor implements IProviderDescriptor
{
     ...
     */
    // public ProviderDescriptor(IConfigurationElement config) throws
    // CoreException {
    ...
    // Map<String, StringMatcher> uriMatcherMap = new java.util.HashMap<String,
    // StringMatcher>();
    //
    ...
    // nsUriMatchers = uriMatcherMap.values().toArray(new
    // StringMatcher[uriMatcherMap.size()]);
    //
    ...
    /**
     * Obtains the evaluation mode of the provider, which indicates the mode of
     * all constraints that it defines.
    ...
    {
        return EvaluationMode.NULL;// mode;
    }
     ...
     * @return whether the provider has any chance of providing constraints for
     *         this context
     */
    @Override
    ...
    {
        return false;// shouldCacheConstraints;
    }
    ...
    /**
     * Obtains my provider. It is lazily instantiated to delay the loading of
     * the contributing plug-in. If, for some reason, the provider cannot be
     * initialized, then a "null provider" is returned which never does
     * anything.
     * @return my provider
     */
    @Override
    public synchronized IModelConstraintProvider getProvider()
    {
        ...
        // ((XmlConstraintProvider) provider).setInitializationData(getConfig(),
        // XmlConfig.A_CLASS,
        // null);
        // } else {
        // provider =
        // (IModelConstraintProvider)
        // getConfig().createExecutableExtension(XmlConfig.A_CLASS);
        // }
        //
        // Trace.trace(EMFModelValidationDebugOptions.PROVIDERS, "Provider
        ...
        // ce);
        //
        // Trace.trace(EMFModelValidationDebugOptions.PROVIDERS, "Provider is
        // disabled. ");//$NON-NLS-1$
        //
    ...
    private boolean providesLiveConstraints(IProviderOperation<? extends Collection<? extends IModelConstraint>> operation)
    {
        ...
        Trace.entering(EMFModelValidationDebugOptions.PROVIDERS,
                       getClass(),
        ...
        // if (isLive()) {
        // GetLiveConstraintsOperation op = (GetLiveConstraintsOperation)
        // operation;
        //
        // if (targets == null) {
    ...
    private boolean providesBatchConstraints(IProviderOperation<? extends Collection<? extends IModelConstraint>> operation)
    {
        ...
        // if (isBatch()) {
        // GetBatchConstraintsOperation op = (GetBatchConstraintsOperation)
        // operation;
        //
        // if (targets == null) {
        // // as a special case, the absence of any "target" elements
    ...
    /**
     * Helper method to determine whether my provider handles the namespace in
     * which an EMF object's type is defined.
     * @param eObject an EMF object
     ...
     * namespace.
     * @param originalTargetNamespace the namespace of the type of the object
     *        being validated
     * @param namespace a namespace for which, perhaps, this provider defines
     *        constraints
     ...
     *        which indicates one of the EMF events for which the provider can
     *        supply constraints
     * @return whether this EMF event type is recognized by my provider
     */
    // private boolean providerHandlesEvent(EMFEventType eventType,
     ...
     * @return <CODE>true</CODE> if my provider specifies live mode or no mode
     *         at all
     */
     ...
     * @return <CODE>true</CODE> if my provider specifies batch mode or no mode
     *         at all
     */
