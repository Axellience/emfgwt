...
public final class EvaluationMode<T> implements Serializable
{
     ...
     * implements a transactional model for changes to the data. The intent is
     * that if any live constraints fail, the transaction may not be committed
     * and may have to be rolled back if the application cannot fix the data.
     * </p>
     * <p>
     ...
     * data integrity, but rather semantic rules that guide a user to creating a
     * better model.
     */
    ...
    /** All valid instances. */
    private static final List<EvaluationMode<?>> instances =
            Collections.unmodifiableList(Arrays.asList(new EvaluationMode<?>[]{LIVE, BATCH, NULL}));
     ...
     * <code>name</code> is not recognized as the name of a valid instance or is
     * <code>null</code>, then the result is the special {@link #NULL} instance.
