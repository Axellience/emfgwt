 ...
 * initial API and implementation Zeligsoft - Bugs 249496, 252302 SAP AG - Bug
 * 240352
 ****************************************************************************/
...
public class ClientContext implements IClientContext
{
    ...
    public ClientContext(String id, final String bindingContributorID)
    {
        ...
        this.selector = new IClientSelector()
        {
            ...
            public boolean selects(Object object)
            {
                ...
                throw new IllegalStateException("NLS.bind(ValidationMessages.binding_noSuchContext_ERROR_, bindingContributorID)");
            }
        };
    }
     ...
     */
    // private String initializeId(IConfigurationElement config) throws
    // CoreException
    // {
     ...
     */
    // private IClientSelector initializeSelector(IConfigurationElement config)
     ...
     */
    // private IClientSelector
    // initializeExpressionSelector(IConfigurationElement enablement)
    // throws CoreException
    // {
    // try {
    ...
    // // (and
    // // final)
    // enablement.getDeclaringExtension()
    // .getNamespaceIdentifier()}),
    // e));
    //
    // Trace.throwing(getClass(), "initializeExpressionSelector", ce);
    // //$NON-NLS-1$
    //
    // throw ce;
    // }
     ...
     * @throws CoreException if something is malformed in the expression
     */
    // private IClientSelector initializeCustomSelector(IConfigurationElement
    // config)
    // throws CoreException
    // {
     ...
     * extend. Note that it is an error for a client-context to, transitively or
     * not, extend itself.
    ...
    private static class BindingFilter
    {
        ...
        boolean getBinding(IConstraintDescriptor constraint)
        {
            return isExcluded(constraint)
                    ? false
                    : isIncluded(constraint) ? true : next().getBinding(constraint);
        }
