...
/**
 * <p>
 * A centralized registry of {@link IConstraintDescriptor descriptors} for the
 * constraints that are available in the validation system. Clients can use this
 * registry to look up the meta-data for constraints (given the constraint IDs),
 * and constraint providers can register the constraints that they provide in
 * the registry.
 * </p>
 * <p>
 * <b>Note</b> for dynamic constraint providers: registering constraints in the
 * registry is not absolutely necessary if none of the following capabilities of
 * the system is required for your constraints:
 * <ul>
 * <li>display in the preference page GUI under the constraint's categories</li>
 * <li>meta-data lookup by other constraints or plug-ins</li>
 * <li>pre-requisite (constraint dependency) controls, replacement/extension of
 * constraints, and other future facilities that require the lookup
 * mechanism</li>
 * </ul>
 * </p>
...
public class ConstraintRegistry
{
    ...
    /**
     * Obtains the unique constraint descriptor having the specified ID. The ID
     * is prepended by the supplied plug-in ID, to generate a fully-qualified ID
     * of the form <tt>&lt;pluginId&gt;.&lt;id&gt;</tt>.
     ...
     * @param descriptor a new constraint descriptor, which must have a unique
    ...
    public synchronized void addConstraintListener(IConstraintListener listener)
    {
        if (indexOf(listener) < 0) {
            if (constraintListeners == null) {
                ...
                System.arraycopy(constraintListeners,
                                 0,
                                 newListeners,
                                 0,
                                 constraintListeners.length);
    ...
    public synchronized void removeConstraintListener(IConstraintListener listener)
    {
        ...
        if (index >= 0) {
            ...
            System.arraycopy(constraintListeners,
                             ...
                             newListeners,
                             index,
                             constraintListeners.length - index - 1);
            constraintListeners = newListeners;
     ...
     * constraint listeners. This method is used internally by constraints to
     * send notifications when they have changed.
     * <p>
    ...
    public void broadcastConstraintChangeEvent(ConstraintChangeEvent event)
    {
        ...
        for (int i = 0; i < array.length; i++) {
            try {
                ...
                if (Trace.shouldTrace(EMFModelValidationDebugOptions.LISTENERS)) {
                    Trace.trace(EMFModelValidationDebugOptions.LISTENERS,
                                "Uncaught exception in constraint listener: " //$NON-NLS-1$
                                                                          + array[i].getClass()
                                                                                    .getName());
                }
    ...
    /**
     * Implements the registration of a constraint. <b>This method requires that
     * the caller synchronize on the <tt>descriptors</tt> map</b>.
