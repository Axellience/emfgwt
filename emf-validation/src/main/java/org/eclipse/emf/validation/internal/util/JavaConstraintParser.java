 ...
 * IBM Corporation - initial API and implementation SAP AG - Bug 240352
 ****************************************************************************/
 ...
 */
public class JavaConstraintParser implements IParameterizedConstraintParser
{
            ...
            throws ConstraintParserException
    {
        ...
        String className =
                descriptor.getParameterValue(IParameterizedConstraintDescriptor.CLASS_PARAMETER);
        ...
        return createCustomConstraint(className,
                                      descriptor.getParameterValue(IParameterizedConstraintDescriptor.BUNDLE_PARAMETER),
                                      descriptor);
    }

    // @SuppressWarnings("deprecation")
    // public IModelConstraint parseConstraint(IXmlConstraintDescriptor
    // descriptor)
    // throws ConstraintParserException
    // {
    //
    // String className =
    // descriptor.getConfig().getAttribute(XmlConfig.A_CLASS);
    //
    // if (className == null) {
    // ConstraintParserException cpe = new ConstraintParserException("No class
    // name."); //$NON-NLS-1$
    //
    // Trace.throwing(getClass(), "parseConstraint", cpe); //$NON-NLS-1$
    // throw cpe;
     ...
     * @param bundleName the symbolic name of the bundle containing the
     *        constraint class
            ...
            throws ConstraintParserException
    {
        ...
        // classProvider = new
        // ClassProvider.BundleProvider(Platform.getBundle(bundleName));
        // } else
        if (descriptor instanceof ModeledConstraintDescriptor) {
            classProvider = ((ModeledConstraintDescriptor) descriptor).getClassProvider();
        } else {
            classProvider =
                    new ClassProvider.ClassLoaderProvider(EMFModelValidationPlugin.INSTANCE);
        }
        ...
        try {
            ...
            if (AbstractModelConstraint.class.isAssignableFrom(resultType)) {
                ...
                result = new ConstraintAdapter(descriptor,
            ...
            pendingException = e;
            pendingMessage =
                    EMFModelValidationPlugin.getMessage(EMFModelValidationStatusCodes.DELEGATE_CLASS_NOT_FOUND_MSG,
            ...
            pendingException = e;
            pendingMessage =
                    EMFModelValidationPlugin.getMessage(EMFModelValidationStatusCodes.DELEGATE_INSTANTIATION_MSG,
                                                        new Object[]{descriptor.getId(),
                                                                     className});
        }
            ...
            pendingException = e;
            pendingMessage =
                    EMFModelValidationPlugin.getMessage(EMFModelValidationStatusCodes.DELEGATE_METHOD_INACCESSIBLE_MSG,
        ...
        if (pendingException != null) {
            ...
            Log.error(EMFModelValidationStatusCodes.CONSTRAINT_NOT_INITED,
                      pendingMessage,
                      pendingException);

            ConstraintParserException cpe =
                    new ConstraintParserException(pendingException.getLocalizedMessage(),
                                                  pendingException);
     ...
     * which should either extend {@link AbstractModelConstraint} or implement
     * an appropriate validation method signature. The instances are pooled to
     * support the sharing of instances as described in the documentation of the
     ...
     * @throws IllegalAccessException if the instance needs to be created and
