 ...
 * Categories can be individually and separately (no dependency on the
 * hierarchy) enabled or disabled by the user. Enablement indicates whether the
 * constraints in the category should be applied to the user's models.
 * </p>
 * <p>
...
public class Category implements Comparable<Category>
{
    ...
    /**
     * The global namespace is the parent of all top-level categories. This
     * ensures that I never have to worry about NULL parents (the global
     ...
     * @param parent my parent category, or <code>null</code> if none (which
     *        should only be the case for the {@link #GLOBAL_NAMESPACE}
     */
    ...
    public final String getQualifiedName()
    {
        if (qualifiedName == null) {
            if (getParent() == null) {
                ...
                qualifiedName = getParent().getQualifiedName() + SLASH + getName();
            }
        }
    ...
    /**
     * Obtains my children.
     * @return an unmodifiable set of the {@link Category}s that are my
     ...
     * <code>descendentPath</code> relative to me. It will be
     * <code>create</code>d, if desired by the caller, if it does not already
     * exist.
    ...
    {
        return (other instanceof Category) && ((Category) other).getPath().equals(getPath());
    }
     ...
     * @param collection the collection of mandatory categories to which I
     *        append
     */
