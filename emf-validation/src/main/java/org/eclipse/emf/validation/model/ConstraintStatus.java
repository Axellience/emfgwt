 ...
 * initial API and implementation Zeligsoft - Bug 249690
 ****************************************************************************/
...
public class ConstraintStatus extends Status implements IConstraintStatus
{
    ...
    /**
     * Initializes me as a failure of the specified <code>constraint</code> with
     * a <code>message</code> to be displayed somehow to the user.
     ...
     */
    public ConstraintStatus(IModelConstraint constraint, EObject target, int severity, int code,
                            String message, Set<? extends EObject> resultLocus)
    {
     ...
     * constraint on the provided target element. The status will have the
     * severity and error code defined in the constraint meta-data (its
     ...
     *        is to report. If this collection does not contain the target
     *        object of the validation, then it is considered to contain it
     *        implicitly. Thus, this parameter may be <code>null</code> if only
     *        the target object is in the result locus
                                                ...
                                                String messagePattern, Object... messageArguments)
    {
        ...
        IConstraintDescriptor desc =
                ConstraintRegistry.getInstance().getDescriptor(ctx.getCurrentConstraintId());

        return createStatus(ctx,
                            target,
                            resultLocus,
                            desc.getSeverity().toIStatusSeverity(),
                            desc.getStatusCode(),
                            messagePattern,
                            messageArguments);
    }
    ...
    public static ConstraintStatus createStatus(IValidationContext ctx, EObject target,
                                                Collection<? extends EObject> resultLocus,
                                                int severity, int errorCode, String messagePattern,
     ...
     * validation context. The status will have the severity and error code
     * defined in the constraint meta-data (its descriptor), and a message
     * composed from the specified pattern and arguments.
     * <p>
     * This is useful in the case that a single constraint object reports
     * multiple distinct problems, with different messages, at the same
     * severity.
     ...
     * constraint on the provided target element. The status will have the
     * severity and error code defined in by
     ...
     * method returns an {@link IConstraintStatus}, otherwise it simply returns
     * the success created by the validation context
     * </p>
     * @param ctx the calling constraint's validation context. Must not be
     *        <code>null</code>
                                              ...
                                              Collection<? extends EObject> resultLocus)
    {
        ...
        if (status instanceof IConstraintStatus) {
            ...
            ConstraintStatus constraintStatus =
                    new ConstraintStatus(successStatus.getConstraint(), target);
     ...
     * <p>
     * This is useful in the case that a single constraint object reports
     * multiple distinct problems. A constraint may choose to compose and return
     * a {@link MultiStatus} if it wants different values for some of the status
     * properties.
     * </p>
     ...
     * severity is, as with all multi-statuses, the maximum of the severities of
     * the supplied <tt>statuses</tt>.
     * <p>
     * This is useful in the case that a single constraint object reports
     * multiple distinct problems. A constraint may choose to compose and return
     * a {@link MultiStatus} if it wants different values for some of the status
     ...
     * @see #createMultiStatus(IValidationContext, Collection)
     */
    public static IStatus createMultiStatus(IValidationContext ctx, String messagePattern,
                                            Object[] messageArguments,
                                            Collection<? extends IStatus> statuses)
    {
     ...
     *         successful validation, the result is an empty collection. The
