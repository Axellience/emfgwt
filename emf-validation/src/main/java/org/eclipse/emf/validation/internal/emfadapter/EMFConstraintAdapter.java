...
public class EMFConstraintAdapter implements IModelConstraint
{
     ...
     * the EMF context serves a similar function to our validation context, but
     * we don't want to expose the map that is implemented in the Aurora
     * validation context. The mapping is weak so that the EMF context can be
     * GCed when the Aurora context is (both contexts only enduring as long as
     * the validation operation). There is no danger of elements in the EMF
     * context holding references to our context, since the EMF APIs have no
     * access to it and there is nothing else in this adapter layer maintains a
     * reference to it.
     */
    ...
    public IStatus validate(IValidationContext ctx)
    {
        ...
        try {
            boolean success = ((Boolean) validationMethod.invoke(ctx.getTarget(),
     ...
     * validation context adapter. Note that the status details are as defined
     * in the Aurora validation XML, not in the Status object provided by the
     * EMF.
    ...
    {
        IStatus result =
                new ConstraintStatus(this, ctx.getTarget(), IStatus.INFO,
                                     getDescriptor().getStatusCode(),
                                     EMFModelValidationPlugin.getMessage("ValidationMessages.emfadapter_disabled_WARN",
