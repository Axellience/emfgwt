 ...
 * initial API and implementation Zeligsoft - Bug 137213 SAP AG - Bug 240352
...
class TraversalStrategyManager
{
     ...
     * strategy provided through the <tt>traversal</tt> extension point, or it
     * may be the default strategy that simply recurses over the entire content
     * tree.
    ...
    /**
     * Initializes the traversal strategies contributed by plug-ins. The
     * traversal strategy manager ensures that plug-in code is only invoked
     * (i.e., traversal strategy implementations instantiated) when required by
    ...
    // // copy on write
    // packageDescriptors = new java.util.HashMap<String,
    // Descriptor>(packageDescriptors);
    //
     ...
     * descriptor has all of the plug-in extension configuration data required
     * to create traversal strategies for the indicated package.
     * <p>
    ...
    private Descriptor getDescriptor(EPackage ePackage)
    {
        ...
        synchronized (traversalsLock) {
            ...
            // if ((rset != null)
            // && (rset.getPackageRegistry().getEPackage(nsURI) == ePackage))
            // {
    ...
    private static class Descriptor
    {
        ...
        /**
         * Determines whether <code>other</code> is a descriptor for the same
         * EPackage as me.
         */
        @Override
        public boolean equals(Object other)
        {
            return (other instanceof Descriptor)
                   && getNamespaceUri().equals(((Descriptor) other).getNamespaceUri());
        }
        ...
        /**
         * Adds a traversal strategy for the EPackage that I handle. This method
         * will not add traversal strategies for EClasses that already have one
         * defined, and likewise for the package's default.
        ...
        //
        // ThreadLocal<ITraversalStrategy> strategy = new
        // ThreadLocalLazyStrategy(config);
        //
         ...
         * extension for the element's EClass (or a superclass thereof), or the
         * default for the package. The default package default is the basic
         * eContents recursion strategy.
        ...
        /**
         * Searches depth-first and left-to-right in the inheritance hierarchy
         * for a strategy that is inherited by the specified
         * <code>eclass</code>.
        ...
        private ThreadLocal<ITraversalStrategy> getInheritedStrategy(EClass eclass)
        {
            ...
            for (Iterator<EClass> iter = eclass.getESuperTypes().iterator(); (result == null)
                                                                             && iter.hasNext();)
            {
         ...
         * specified <code>config</code>uration element. This is only done when
         * the strategy is actually needed to perform a validation.
         * @param config the traversal strategy extension declaration
         * @return the traversal strategy declared by the <code>config</code>,
         *         or the default eContents recursion strategy if a problem
         *         occurred in instantiating it
         */
        // private ITraversalStrategy initializeStrategy(IConfigurationElement
        // config)
        // {
        // ITraversalStrategy result;
        //
        // try {
        // result = (ITraversalStrategy)
        // config.createExecutableExtension(A_CLASS);
        // }
        ...
        // }
        //
        // private class ThreadLocalLazyStrategy extends
        // ThreadLocal<ITraversalStrategy>
        // {
        ...
        // {
        // return (config == null)
        // ? new ITraversalStrategy.Recursive()
        // : initializeStrategy(config);
        // }
        //
        // void preinitialize(ITraversalStrategy strategy, IConfigurationElement
        // config)
        // {
        ...
        /**
         * An adapter that removes the descriptor for a dynamic EPackage when
         * that EPackage is unloaded.
         */
