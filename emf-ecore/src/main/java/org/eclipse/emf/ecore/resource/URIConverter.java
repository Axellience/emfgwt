 ...
 * {@link org.eclipse.emf.ecore.resource.impl.ExtensibleURIConverterImpl
...
{
    /**
     * An option used to pass the calling URIConverter to the
     * {@link URIHandler}s.
     * @since 2.4
     */
     ...
     * the result for the asynchronous methods.
     * @since 2.7
     */
     ...
     * used by resource {@link Resource#load(Map) load} and
     * {@link Resource#save(Map) save} in order to set the
     * {@link Resource#getTimeStamp()}.
     * @since 2.4
     */
    ...
    /**
     * A property of the {@link #OPTION_RESPONSE response option} used to yield
     * the newly allocated URI associated with the creation of an
     * {@link #createOutputStream(URI, Map) output} stream. This is typically
     * used by resource {@link Resource#save(Map) save} in order to
     * {@link Resource#setURI(URI) set the resource URI}.
     * @since 2.7
     */
    String RESPONSE_URI = "URI";

    /**
     * An option to pass a timeout value, i.e., a non-negative
     * <code>Integer</code>, to the URI converter's methods. If the operation
     * cannot be completed within the specified number of milliseconds and the
     * {@link #getURIHandler(URI) handler} for the URI supports timeouts, an
     * {@link IOException} is thrown, e.g., a URI with <code>http</code> scheme
     * will throw {@link SocketTimeoutException}. The default value is
     * <code>0</code>, i.e., infinite timeout.
     * @since 2.9
     */
     ...
     * {@link #store(URI, byte[], Map, Callback) store}, or
     * {@link #delete(URI, Map, Callback) delete} option that specifies a long
     * {@link #RESPONSE_TIME_STAMP_PROPERTY timestamp} which must match the
     * underlying resource's timestamp for the update to succeed.
     * @since 2.7
     */
     ...
     * </p>
     * <p>
     * It is important to emphasize that normalization can result in loss of
     ...
     * {@link URIConverter#URI_MAP global} map, so registrations made in this
     ...
     * (typically) be available for {@link URIConverter#normalize use} by any
     * URI converter. It is populated by URI mappings registered via
     * {@link org.eclipse.emf.ecore.plugin.EcorePlugin.Implementation#startup()
     * plugin registration}.
     * @see #normalize(URI)
     */
    Map<URI, URI> URI_MAP =
            org.eclipse.emf.ecore.resource.impl.URIMappingRegistryImpl.INSTANCE.map();
     ...
     * URI handlers which {@link URIHandler#canHandle(URI) can handle} the given
     * URI.
     ...
     * </p>
     * @param uri the URI for which to create the input stream.
     * @param options a map of options to influence the kind of stream that is
     *        returned; unrecognized options are ignored and <code>null</code>
     *        is permitted.
     * @return an open input stream.
     * @exception IOException if there is a problem obtaining an open input
     *            stream.
     ...
     * from {@link URIConverter#createInputStream(URI)} and
     * {@link URIConverter#createInputStream(URI, Map)}. An input stream
     * implementing this interface is highly unlikely to support
     * {@link InputStream#read() read}. Instead {@link #loadResource(Resource)
     * loadResource} should be called.
     * @since 2.7
     */
    interface Loadable
    {
        /**
         * Load the contents of the resource directly from the backing store for
         * which the stream implementing this interface is a facade.
         * @param resource the resource to load.
         * @throws IOException if there are any problems load the resource from
         *         the backing store.
         */
    ...
    /**
     * An interface that is optionally implemented by the output streams
     * returned from {@link URIConverter#createOutputStream(URI)} and
     * {@link URIConverter#createOutputStream(URI, Map)}. An output stream
     * implementing this interface is highly unlikely to support
     * {@link OutputStream#write(int) write}. Instead
     * {@link #saveResource(Resource) saveResource} should be called.
     * @since 2.7
     */
    interface Saveable
    {
        /**
         * Save the contents of the resource directly to the backing store for
         * which the stream implementing this interface is a facade.
         * @param resource the resource to save.
         * @throws IOException if there are any problems saving the resource to
         *         the backing store.
         */
    ...
    interface Cipher
    {
         ...
         * allowing the Cipher implementation to do any maintenance work
         * required, such as flushing an internal cache.
         * @param outputStream the encrypted stream returned by
         *        {@link #encrypt(OutputStream)}.
         */
         ...
         * allowing the Cipher implementation to do any maintenance work
         * required, such as flushing internal cache.
         * @param inputStream the stream returned by
         *        {@link #decrypt(InputStream)}.
         */
     ...
     * @param options options to influence how the contents are deleted, or
     ...
     * @param uri the URI to consider.
     * @param options options to influence how the content description is
     ...
     * @param uri the URI to consider.
     * @param options options to influence how the existence determined, or
     *        <code>null</code> if there are no options.
     ...
     * the result.
     * @param uri the URI to consider.
     * @param options options to influence how the attributes are determined, or
     *        <code>null</code> if there are no options.
     ...
     * @param options options to influence how the attributes are updated, or
     *        <code>null</code> if there are no options.
    ...
    /**
     * The global static URI converter instance. It's generally not a good idea
     * to modify any aspect of this instance. Instead, use a resource set's
     * {@link ResourceSet#getURIConverter() local} instance.
     * @since 2.4
     */
