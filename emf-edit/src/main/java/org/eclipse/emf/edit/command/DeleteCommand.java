 ...
 * additionally deletes all other references to it from within the editing
 * domain.
 * 
 * @since 2.2
 */
public class DeleteCommand extends CompoundCommand
{
    ...
    {
        return domain.createCommand(DeleteCommand.class,
    ...
    protected static final String LABEL =
            EMFEditPlugin.INSTANCE.getString("_UI_DeleteCommand_label");

    /**
     * This caches the description.
     */
    protected static final String DESCRIPTION =
            EMFEditPlugin.INSTANCE.getString("_UI_DeleteCommand_description");
     ...
     * be cleared.
     * The default implementation uses {@link UsageCrossReferencer} to find all
    ...
    {
        ECrossReferenceAdapter crossReferenceAdapter = getCrossReferenceAdapter();

        if (crossReferenceAdapter != null) {
            Map<EObject, Collection<EStructuralFeature.Setting>> result = new HashMap<>();
            
            for (EObject eObject : eObjects) {
                result.put(eObject, crossReferenceAdapter.getInverseReferences(eObject, false));
            }
    ...
    private ECrossReferenceAdapter getCrossReferenceAdapter()
    {
        for (Adapter adapter : domain.getResourceSet().eAdapters()) {
