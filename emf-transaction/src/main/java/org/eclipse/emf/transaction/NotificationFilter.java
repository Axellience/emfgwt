 ...
 * implementation Zeligsoft - Bug 225711 </copyright> $Id:
 * NotificationFilter.java,v 1.4 2008/09/14 02:38:08 cdamus Exp $
 */
 ...
 * {@link ResourceSetListener}s. Filters can be combined using simple boolean
 * operations.
 * <p>
 ...
 * intended only to inform an adapter that it is being removed; they are not
 * broadcast to all adapters of an object. Besides which, resource set listeners
 * are not adapters.
 * </p>
 * <p>
 * Since the EMF Transaction 1.3 release, clients can implement arbitrary filter
 * criteria by specializing the {@link NotificationFilter.Custom} class.
 * </p>
...
public abstract class NotificationFilter
{
    ...
    /**
     * Matches any notification that can occur during the normal course of a
     * read-only transaction. These include:
     * <ul>
     * <li>proxy resolution</li>
     * <li>any change to the {@link ResourceSet#getResources() resources} list
     * of a resource set</li>
     ...
     * <li>any change to the {@link Resource#getContents() contents} of a
     * resource while it is loading or unloading</li>
     * </ul>
     */
     ...
     * specified content type. Because the determination of a resource's content
     * type is costly, it is cached on the resource via an adapter and is
     * cleared on any change to the resource's contents or other properties.
     ...
     * @return the filter
     */
    // public static NotificationFilter createFeatureFilter(final Class<?>
    // ownerType,
    // final int featureId)
    // {
    // return new NotificationFilter()
     ...
     * @return the filter
     */
    public static NotificationFilter createFeatureFilter(final EClassifier ownerType,
                                                         final int featureId)
    {
        return new NotificationFilter()
        {
     ...
     * specified type. This variant is useful for notifiers that are not modeled
     * via Ecore.
     * @param type the notifier type as a Java class or interface
     * @return the filter
     */
    // public static NotificationFilter createNotifierTypeFilter(final Class<?>
    // type)
    // {
    // return new NotificationFilter()
    // {
    ...
    /**
     * Creates a filter matching notifications from any instance of the
     * specified type. This variant is useful for notifiers that are modeled via
     * Ecore.
     * @param type the notifier type as an Ecore classifier
     * @return the filter
     */
    // public static NotificationFilter createNotifierTypeFilter(final
    // EClassifier type)
    // {
    // return new NotificationFilter()
    // {
