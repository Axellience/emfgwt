 ...
 * implementation Zeligsoft - Bug 245446 </copyright> $Id: Transaction.java,v
 * 1.9 2008/12/03 14:48:13 cdamus Exp $
 */
...
public interface Transaction
{
    ...
    /**
     * Option to suppress the pre-commit event that implements triggers. This
     * does not suppress the post-commit event.
     * <p>
     * The value is a {@link Boolean}; the default is {@link Boolean#FALSE}.
     * </p>
     */
     ...
     * so a transaction could still roll back on commit if a pre-commit listener
     * throws.
     * <p>
     * The value is a {@link Boolean}; the default is {@link Boolean#FALSE}.
     * </p>
    ...
    /**
     * Option to suppress undo/redo recording. This has two effects: it prevents
     * rollback of the transaction, as this requires the undo information. It
     * also prevents undo/redo of any {@link RecordingCommand}s executed in the
     ...
     * suppresses undo recording, triggers, and validation. It does not suppress
     * post-commit events.
     * <p>
     * The value is a {@link Boolean}; the default is {@link Boolean#FALSE}.
     * </p>
     ...
     * transaction API. Clients may only check to see whether a transaction has
     * this option, e.g., in a
    ...
    /**
     * Queries the editing domain in which I am transacting. Note that this is
     * available also before I start and after I close.
     ...
     * active transaction can create nested transactions simply by starting new
     * ones. Nested transactions commit differently from top-level transactions:
     * although they send
     * {@link ResourceSetListener#transactionAboutToCommit(ResourceSetChangeEvent)
     * pre-commit} events, they do not send post-commit events, nor are they
     * validated. Validation is performed only by the top-level transaction to
     * validate all changes made in its scope, and only the top-level
     * transaction then can send the post-commit event. Nested transactions can
     * roll back their changes without affecting their parent transactions.
     * <p>
     * Transactions can nest as follows:
     * </p>
     * <ul>
     * <li>read-only transactions can be nested in read-only or read/write
     * transactions</li>
    ...
    /**
     * Queries the thread that owns me. Only this thread is allowed to read or
     * write (in the case of read/write transactions) the editing domain's
    ...
    /**
     * Queries whether I am a read-only transaction. Even my owning thread is
     * not permitted to make changes to the model if I am read-only.
     * @return <code>true</code> if I am read-only; <code>false</code>,
     *         otherwise
     */
    boolean isReadOnly();

    /**
     * Obtains the special options with which I was created. The options (map
     * keys) are defined by the {@link #OPTION_NO_NOTIFICATIONS Transaction}
     * interface.
    ...
    /**
     * Queries whether I am active. I am active after I have started and before
     * I have closed (committed or rolled back).
     ...
     * {@link TransactionalEditingDomain} supports any number of
     * pseudo-concurrent read-only transactions. Transactions that are expected
     * to be long-running should yield frequently, as a task running in a
     * progress monitor is expected to check for cancellation frequently.
     * However, there is a higher cost (in time) associated with yielding, so it
     * should not be overdone.
     * <p>
     * Only read-only transactions may yield, and only the transaction that is
     * currently active in the editing domain may yield. The yielding
     * transaction may be nested, but not within a read/write transaction at any
     * depth.
     * </p>
     * <p>
     ...
     * introduce dirty reads (transactions reading uncommitted changes). If
     * there are no other read-only transactions to receive the transfer of
    ...
    /**
     * Attempts to commit the transaction. The transaction may only commit if it
     * is the currently active transaction in the editing domain. After the
     ...
     * committing. If any listener throws a {@link RollbackException}, then the
     * transaction is rolled back and the exception is propagated to the caller.
     * Any trigger commands returned by pre-commit listeners are executed after
     * all listeners are invoked, in a nested transaction. This nested
     * transaction, then, follows the commit protocol to send out pre-commit
     * notifications again. This process continues until no more trigger
     * commands are executed or some listener causes rollback.
     * </p>
     * <p>
     * After all pre-commit processing completes, the transaction is validated.
     * Validation checks all of the notifications received from the model during
     * the transaction (including any nested transactions, esp. those that
     * executed triggers). If the validation yields an error status (or more
     * severe), then the transaction is rolled back, throwing a
     ...
     * notifications received during the transaction, including triggers. Note
     * that, because these listeners can read the model, they may cause further
     * notifications (by resolving proxies, loading resources, etc.). Listeners
     * are invoked in a nested read-only transaction, so it will also commit and
     * send out a post-commit event if necessary with additional notifications.
     * </p>
     * <p>
     ...
     * be started again. No events are sent when the transaction rolls back; to
     * listeners it appears that nothing ever happened.
     */
    void rollback();

    /**
     * Obtains the change description summarizing the changes made to the model
     * during the execution of the transaction. The change description must not
     * be used until after the transaction has successfully committed. If the
     * transaction rolls back, then it has no change description.
    ...
    /**
     * Obtains the status of the transaction. This may provide warning or or
     * error messages from validation (after I have committed/rolled back) or
     * other sources, or it might be OK.
     * <p>
     * <b>Note</b> that while I am still active, my status is usually OK. It may
     * not be OK if I have been aborted, in which case I will roll back on
     * attempt to commit.
     * </p>
    ...
    interface OptionMetadata
    {
         ...
         * @param options1 an options map
         * @param options2 another options map
         ...
         * @param parentOptions the options map to inherit a value from. It is
         *        conceivable that inheritance of an option may depend on more
         *        than one option in this parent map
         * @param childOptions the map that is to inherit the option setting
         * @param force whether to inherit the option anyway despite that it is
         *        not hereditary. This is used for application of default
         *        options, and can be ignored by the implementor if necessary.
         *        Also, clients must not use this parameter to attempt to
         *        override an existing child setting; a well-behaved option will
         *        not do that
         */
        ...
        interface Registry
        {
            ...
            Registry INSTANCE = new BasicTransactionOptionMetadataRegistry(null)
            {
...

                {
                    ...
                    register(BasicTransactionOptionMetadata.newBoolean(Transaction.OPTION_NO_NOTIFICATIONS,
                                                                       false));
                    register(BasicTransactionOptionMetadata.newBoolean(Transaction.OPTION_NO_TRIGGERS,
                                                                       false));
                    register(BasicTransactionOptionMetadata.newBoolean(Transaction.OPTION_NO_VALIDATION,
                                                                       false));
                    register(BasicTransactionOptionMetadata.newBoolean(Transaction.OPTION_NO_UNDO,
                                                                       false));
                    register(BasicTransactionOptionMetadata.newBoolean(Transaction.OPTION_UNPROTECTED,
                                                                       false));
                    register(BasicTransactionOptionMetadata.newBoolean(Transaction.OPTION_IS_UNDO_REDO_TRANSACTION,
                                                                       false));

                    register(new ValidateEditOption());
                    register(new BasicTransactionOptionMetadata(Transaction.OPTION_VALIDATE_EDIT_CONTEXT));

                    register(BasicTransactionOptionMetadata.newBoolean(TransactionImpl.OPTION_IS_TRIGGER_TRANSACTION,
                                                                       false));
                    register(new AllowChangePropagationBlockingOption());
                    register(new BlockChangePropagationOption());
                    register(new BasicTransactionOptionMetadata(TransactionImpl.OPTION_EXECUTING_COMMAND,
                                                                true, false, Command.class, null));
                }
            };
