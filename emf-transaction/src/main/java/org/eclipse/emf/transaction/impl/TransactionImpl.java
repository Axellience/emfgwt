 ...
 * implementation Zeligsoft - Bugs 145877, 250253, 245446 </copyright> $Id:
 * TransactionImpl.java,v 1.22 2008/11/30 16:38:08 cdamus Exp $
 */
...
public class TransactionImpl implements InternalTransaction
{
    ...
    /**
     * This option, when provided to a transaction that inherits from this
     * implementation class and has children transactions that are using this
     * implementation class, provides an optional block of the normal
     * propagation of change descriptions to the parent transaction by any
     * transaction in the child subtree of this transaction. The child exercises
     * its option to negate the propagation of change descriptions by adding the
     * {@link #BLOCK_CHANGE_PROPAGATION} option to its own options with the
     * value of {@link Boolean#TRUE}. This option <i>IS</i> inherited by child
     * transactions.
     */
    public static final String ALLOW_CHANGE_PROPAGATION_BLOCKING = "allow_block_cd_prop"; //$NON-NLS-1$

    /**
     * This option blocks the propagation of change descriptions to the parent
     * transaction. The option has no effect unless the parent transaction has
     * allowed this negation to happen by having the
     * {@link #ALLOW_CHANGE_PROPAGATION_BLOCKING} option added either directly
     * or through option inheritance. Note that to enable this option it must be
     * added to the options map with the value of {@link Boolean#TRUE}. This
     * option is <i>NOT</i> inherited by child transactions.
     */
     ...
     * Moreover, validation is not required during undo/redo because we can only
     * return the model from a valid state to another valid state if the
     * original execution did so. Finally, it is not necessary to record undo
     * information when we are undoing or redoing.
     * <p>
    ...
    private boolean                            closing; // prevents re-entrant
                                                        // commit/rollback
    ...
    public synchronized void start() throws InterruptedException
    {
        ...
        // IllegalStateException exc = new IllegalStateException("Not
        // transaction owner"); //$NON-NLS-1$
        ...
        if (this != getInternalDomain().getActiveTransaction()) {
            IllegalStateException exc =
                    new IllegalStateException("Activated transaction while another is active"); //$NON-NLS-1$
            Tracing.throwing(TransactionImpl.class, "start", exc); //$NON-NLS-1$
            throw exc;
        }
        ...
        if (Tracing.shouldTrace(EMFTransactionDebugOptions.TRANSACTIONS)) {
            ...
            Tracing.trace("*** Started " //$NON-NLS-1$
                          + TransactionalEditingDomainImpl.getDebugID(this)
                          + " read-only=" //$NON-NLS-1$
                          + isReadOnly()
                          // + " owner=" //$NON-NLS-1$
                          // + getOwner().getName()
        ...
        if (getRoot() == this) {
            ...
            if (validateEdit instanceof ValidateEditSupport) {
                getInternalDomain().getChangeRecorder()
                                   .setValidateEditSupport((ValidateEditSupport) validateEdit);
            }
        }
    ...
    {
        return TransactionUtil.getAdapter(getEditingDomain(), InternalLifecycle.class);
    }
     ...
     * @return <code>true</code> if I have been aborted; <code>false</code>,
     *         otherwise
    ...
    public void commit() throws RollbackException
    {
        ...
        if (Tracing.shouldTrace(EMFTransactionDebugOptions.TRANSACTIONS)) {
            Tracing.trace("*** Committing " //$NON-NLS-1$
                          + TransactionalEditingDomainImpl.getDebugID(this)
                          + " at " //$NON-NLS-1$
                          + Tracing.now());
        }
        ...
        try {
            ...
            // first, check whether I have been aborted. If so, then I must roll
            // back
            if (isAborted()) {
                doRollback();
     ...
     * @param validationStatus a live-validation status, or <code>null</code> if
    ...
    private IStatus combine(IStatus validationStatus, IStatus editStatus)
    {
        ...
        if ((validationStatus == null) || validationStatus.isOK()) {
            ...
            result = new MultiStatus(EMFTransactionPlugin.getPluginId(), editStatus.getCode(),
                                     new IStatus[]{editStatus, validationStatus},
                                     editStatus.getMessage(), null);
        } else {
            // live-validation status has priority
            result = new MultiStatus(EMFTransactionPlugin.getPluginId(), validationStatus.getCode(),
    ...
    private void doRollback()
    {
        ...
        if (Tracing.shouldTrace(EMFTransactionDebugOptions.TRANSACTIONS)) {
            Tracing.trace("*** Rolling back " //$NON-NLS-1$
                          + TransactionalEditingDomainImpl.getDebugID(this)
                          + " at " //$NON-NLS-1$
                          + Tracing.now());
        }
        ...
        try {
            ...
            if ((status == null) || status.isOK()) {
                ...
                setStatus(new Status(IStatus.ERROR, EMFTransactionPlugin.getPluginId(),
                                     "Messages.rollbackRequested"));
            } else if (status.getSeverity() < IStatus.ERROR) {
                // make a multi-status for rollback error but preserving the
                // original status
                IStatus rbStatus = new Status(IStatus.ERROR, EMFTransactionPlugin.getPluginId(),
                                              "Messages.rollbackRequested");

                setStatus(new MultiStatus(EMFTransactionPlugin.getPluginId(), rbStatus.getCode(),
                                          new IStatus[]{rbStatus, status}, rbStatus.getMessage(),
                                          null));
            }
        }
        finally {
     ...
     * transaction, I
     * {@link InternalTransaction#resume(TransactionChangeDescription) resume}
     * it.
     * <p>
    ...
    protected synchronized void close()
    {
        if (isActive()) {
            ...
            if (parent != null) {
                // my parent resumes recording its changes now that mine are
                // either
            ...
            if (Tracing.shouldTrace(EMFTransactionDebugOptions.TRANSACTIONS)) {
                Tracing.trace("*** Closed " //$NON-NLS-1$
                              + TransactionalEditingDomainImpl.getDebugID(this)
                              + " at " //$NON-NLS-1$
                              + Tracing.now());
            }
     ...
     * @return the result of validation. If this is an error or worse, then I
     *         must roll back
     */
    protected IStatus validate()
    {
        if (Tracing.shouldTrace(EMFTransactionDebugOptions.TRANSACTIONS)) {
            Tracing.trace("*** Validating " //$NON-NLS-1$
                          + TransactionalEditingDomainImpl.getDebugID(this)
                          + " at " //$NON-NLS-1$
                          + Tracing.now());
        }
    ...
    {
        // extract out only the triggered commands (exclude the triggering
        // command)
        ...
        triggerCommand =
                new ConditionalRedoCommand.Compound(new java.util.ArrayList<Command>(triggerCommands));
    ...
    public void startPrivileged(PrivilegedRunnable<?> runnable)
    {
        if (runnable.getTransaction() != this) {
            throw new IllegalArgumentException("runnable has no privileges on this transaction"); //$NON-NLS-1$
        }
        ...
        if (!isActive() || (internalDomain.getActiveTransaction() != this)) {
            throw new IllegalStateException("transaction is not the domain's current transaction"); //$NON-NLS-1$
        }
    ...
    private void inheritOptions(Transaction parent)
    {
        ...
        Map<?, ?> parentOptions =
                isRoot ? getDefaultOptions(getEditingDomain()) : parent.getOptions();

        if (parentOptions != null) {
            Transaction.OptionMetadata.Registry reg =
                    TransactionUtil.getTransactionOptionRegistry(getEditingDomain());

            for (Object option : parentOptions.keySet()) {
                reg.getOptionMetadata(option).inherit(parentOptions, mutableOptions, isRoot);
            }
        }
    }
    ...
    {
        return "Transaction[active=" //$NON-NLS-1$
               + isActive()
               + ", read-only=" //$NON-NLS-1$
               + isReadOnly()
               // + ", owner=" //$NON-NLS-1$
     ...
     * {@link Transaction#isReadOnly() read-only state}.
     * @param tx a transaction
     * @return <code>true</code> if the transaction should validate changes;
     ...
     * {@link Transaction#isReadOnly() read-only state}.
     * @param tx a transaction
     * @return <code>true</code> if the transaction should perform the
     *         pre-commit procedures; <code>false</code>, otherwise
     */
    ...
    {
        return !tx.isReadOnly() && hasOption(tx, OPTION_UNPROTECTED);
    }

    /**
     * Queries whether the specified transaction collects notifications for
     * broadcast to listeners or for validation. This is determined by the
    ...
    {
        return isNotificationEnabled(tx) || isTriggerEnabled(tx) || isValidationEnabled(tx);
    }
    ...
    protected static Map<?, ?> getDefaultOptions(TransactionalEditingDomain domain)
    {
        ...
        TransactionalEditingDomain.DefaultOptions defaults =
                TransactionUtil.getAdapter(domain, TransactionalEditingDomain.DefaultOptions.class);
