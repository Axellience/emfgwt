/**
 * <copyright> Copyright (c) 2005, 2008 IBM Corporation and others. All rights
 * reserved. This program and the accompanying materials are made available
 * under the terms of the Eclipse Public License v1.0 which accompanies this
 * distribution, and is available at http://www.eclipse.org/legal/epl-v10.html
 * Contributors: IBM - Initial API and implementation </copyright> $Id:
 * ReadWriteValidatorImpl.java,v 1.12 2008/04/17 16:36:17 cdamus Exp $
 */
 ...
 * which notifications were received during which transaction, and at which time
 * relative to the start and completion of nested transactions.
 * </p>
 * <p>
 * Whenever a transaction (nested or otherwise) is rolled back, it must be
 * {@link #remove(InternalTransaction) removed} from me so that I may forget the
 * notifications received for any changes that it or its nested transactions
 * made.
 * </p>
...
public class ReadWriteValidatorImpl implements TransactionValidator
{
    ...
    public void add(InternalTransaction transaction)
    {
        ...
        if (notificationMask > 0) {
            ...
            if (newTree != null) {
                ...
                // next phase of aggregated precommit must start with this
                // transaction
    ...
    /**
     * Removes the specified transaction from me. This must be called when the
     * transaction is rolled back, and is recommended also after a successful
     * commit.
     */
    @Override
    ...
    public synchronized List<Notification> getNotificationsForPrecommit(Transaction tx)
    {
        ...
        if ((transactionToPrecommit != null) && (transactionToPrecommit == findTree(tx))) {
     ...
     * @return the corresponding notification tree node, or <code>null</code> if
     *         this transaction has not yet been added to me or has already
     *         completed (in which case, it is no longer in my map)
     */
    ...
    public IStatus validate(Transaction tx)
    {
        ...
        try {
            ...
            result = new Status(IStatus.ERROR, EMFTransactionPlugin.getPluginId(),
                                EMFTransactionStatusCodes.VALIDATION_FAILURE, "Validation Failure",
                                e);
            // Messages.validationFailure, e);
        }
    ...
    /**
     * Creates a validator which will be responsible for the transaction
     * validation
     * @since 1.1
    ...
    /**
     * A tree mirroring the nesting structure of transactions. The tree records,
     * for every transaction:
     * <ul>
     * <li>the notifications (by directly referencing the mutable notification
     * list)</li>
     * <li>tree nodes for corresponding to the the children of the transaction
     * (transactions otherwise only know their parents)</li>
     * <li>the number of notifications in the parent transaction that preceded
     * its activation, if it has a parent</li>
     * <li>a bit mask indicating which kinds of notifications (pre/post commit
     * and validation) the transaction provides</li>
     * </ul>
     * <p>
     * The third item above is important in reconstructing the complete ordering
     * (in linear time) of the notifications received during nesting
     * transactions, so that both validation and post-commit listeners get the
    ...
    private static class NotificationTree
    {
        ...
        /**
         * Initializes a new tree node for the specified transaction. If it has
         * a parent, then it records the number of notifications that the parent
         * has so far collected.
         * @param transaction the transaction
         * @param parent the tree node of the parent transaction, or
         *        <code>null</code> if there is no parent node
         * @param notificationMask a mask of the {@link #POSTCOMMIT},
         *        {@link #PRECOMMIT}, and {@link #VALIDATION} bits of the kinds
         *        of notifications that the transaction collects
         */
        NotificationTree(InternalTransaction transaction, NotificationTree parent,
                         byte notificationMask)
        {
         ...
         * @param purpose a bit indicating what kind of notifications to collect
         *        (for what purpose we are collecting them)
        ...
        /**
         * Recursive implementation of the {@link #collectNotifications()}
         * method.
        ...
        private void collectNotifications(List<? super Notification> notifications, byte purpose)
        {
            if ((notificationMask & purpose) == purpose) {
                ...
                for (NotificationTree next : children) {
                    ...
                    notifications.addAll(parentNotifications.subList(lastIndex,
        ...
        /**
         * Indicates that my transaction has been rolled back. This will reduce
         * the list of notifications that I store to only those indicating
        ...
        void setRolledBack()
        {
            ...
            if (inPlace) {
                ...
                notifications =
                        new org.eclipse.emf.common.util.BasicEList.FastCompare<Notification>();
            }
         ...
         * change to a model element. This filters out non-model changes such as
         * changes to the modification/loaded state of resources, their URIs,
         * etc.
        ...
        {
            return (notifications != null) ? notifications : transaction.getNotifications();
        }
