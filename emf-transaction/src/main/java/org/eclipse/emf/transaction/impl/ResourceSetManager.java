/**
 * <copyright> Copyright (c) 2005, 2007 IBM Corporation and others. All rights
 * reserved. This program and the accompanying materials are made available
 * under the terms of the Eclipse Public License v1.0 which accompanies this
 * distribution, and is available at http://www.eclipse.org/legal/epl-v10.html
 * Contributors: IBM - Initial API and implementation </copyright> $Id:
 * ResourceSetManager.java,v 1.4 2007/12/04 23:04:49 cdamus Exp $
 */
...
public final class ResourceSetManager
{
    ...
    private final Map<Resource, Boolean> loadingResources   =
            new java.util.HashMap<Resource, Boolean>();
    private final Map<Resource, Boolean> loadedResources    =
            new java.util.HashMap<Resource, Boolean>();
    private final Map<Resource, Boolean> unloadingResources =
            new java.util.HashMap<Resource, Boolean>();
        ...
        if (notification.getFeatureID(null) == ResourceSet.RESOURCE_SET__RESOURCES) {
            ...
            switch (notification.getEventType()) {
                ...
                case Notification.UNSET:
                    ...
                    if (oldValue != null) {
                        // the old resource is effectively unloaded as far as we
                        // are
                        // concerned because it is no longer in this resource
                        // set
                ...
                case Notification.REMOVE_MANY:
                    if (oldValue != null) {
                        ...
                        for (Resource next : resources) {
                            // the old resource is effectively unloaded as far
                            // as we are
                            // concerned because it is no longer in this
                            // resource set
                            setUnloaded(next);
                        }
                    }
        ...
        switch (notification.getFeatureID(null)) {
            ...
            case Resource.RESOURCE__CONTENTS:
                if (!isLoaded(res) && res.isLoaded()) {
                    // we must be in the process of loading this resource if we
                    // didn't
                    // think the resource is loaded yet, but it thinks it is
                    setLoading(res);
                } else if (isLoaded(res) && !res.isLoaded()) {
                    // we must be in the process of unloading this resource if
                    // we
     ...
     * @param res a resource
     * @return <code>true</code> if the resource is not yet loaded but is in the
     *         process of loading; <code>false</code>, otherwise
     */
     ...
     * @param res a resource
     * @return <code>true</code> if the resource is loaded but is in the process
     *         of unloading; <code>false</code>, otherwise
     */
