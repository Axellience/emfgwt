 ...
 * implementation Zeligsoft - Bugs 218276, 245446 </copyright> $Id:
 * RecordingCommand.java,v 1.10 2009/08/19 09:37:24 bgruschko Exp $
 */
...
public abstract class RecordingCommand extends AbstractCommand implements ConditionalRedoCommand
{
        ...
        if ((isNestedCommand() || isTriggerCommand()) && isUndoable()) {
            ...
            try {
                ...
                internalDomain.getActiveTransaction()
                              .abort(new Status(IStatus.ERROR, EMFTransactionPlugin.getPluginId(),
     ...
     * @throws IllegalStateException if I am not {@linkplain #canUndo()
     *         undoable}
     * @see #canUndo()
     */
    @Override
     ...
     * override this method.
     * @throws IllegalStateException if I am not {@linkplain #canRedo()
     *         redoable}
     * @see #canRedo()
     */
    @Override
    ...
    private boolean isTriggerCommand()
    {
        ...
        while (!result && (tx != null)) {
            result = Boolean.TRUE.equals(tx.getOptions()
                                           .get(TransactionImpl.OPTION_IS_TRIGGER_TRANSACTION));
            tx = tx.getParent();
        }
