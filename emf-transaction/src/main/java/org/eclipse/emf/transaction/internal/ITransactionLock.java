...
public interface ITransactionLock
{
    ...
    /**
     * Acquires me, waiting as long as necessary or until I am interrupted. if I
     * already own this lock, then its lock depth is increased. That means one
     * more call to {@link #release()} for me to make.
     * <p>
     ...
     * guaranteed because every thread waits on a different object that is not
     * me, so my monitor is <b>not</b> released when the calling thread blocks.
     * </p>
     * @param exclusive <code>true</code> if the current thread needs exclusive
     *        access (i.e., no other threads may currently be {@link #yield()
     *        yielding} me); <code>false</code>, otherwise
     ...
     * me, so my monitor is <b>not</b> released when the calling thread blocks.
     * </p>
     * @param timeout the number of milliseconds to wait before giving up on the
     *        lock, or <code>0</code> to wait as long as necessary
     * @param exclusive <code>true</code> if the current thread needs exclusive
     *        access (i.e., no other threads may currently be {@link #yield()
     *        yielding} me); <code>false</code>, otherwise
     ...
     * @throws InterruptedException on interruption of the calling thread
     */
    public abstract boolean acquire(long timeout, boolean exclusive) throws InterruptedException;
     ...
     * <p>
     * If this method is called from a thread that is running as a Job, then it
     * behaves identically to {@link #acquire(boolean)}.
     * </p>
     * <p>
     * <b>Note:</b> The current thread must not own my monitor when it calls
     * this method, otherwise it will cause deadlock. Deadlock would be
     ...
     *        access (i.e., no other threads may currently be {@link #yield()
     *        yielding} me); <code>false</code>, otherwise
     * @throws InterruptedException in case of interrupt while waiting or if the
     *         user cancels the lock-acquisition job that is blocking the UI
     *         thread
     */
    public abstract void uiSafeAcquire(boolean exclusive) throws InterruptedException;

    /**
     * Releases me. Note that my depth may still be positive, in which case I
     * would need to be released again (recursively).
     ...
     * {@linkplain #release() release} me before another thread can take me. It
     * then resumes by acquiring me again, layer.
