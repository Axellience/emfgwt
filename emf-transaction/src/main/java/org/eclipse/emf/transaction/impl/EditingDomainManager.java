 ...
 * implementation Zeligsoft - Bug 145877 </copyright> $Id:
 * EditingDomainManager.java,v 1.6 2008/09/20 21:23:08 cdamus Exp $
 */
 ...
 * <code>org.eclipse.emf.transaction.editingDomains</code> extension point. It
 * also configures listeners from the
...
public class EditingDomainManager
{
    ...
    private final Map<String, ResourceSetListener> listeners =
            new java.util.HashMap<String, ResourceSetListener>();
     ...
     * @return the corresponding editing domain, or <code>null</code> if no such
     *         extension was found
     */
    public TransactionalEditingDomain createEditingDomain(String id)
    {
        ...
        // result = ((TransactionalEditingDomain.Factory)
        ...
        // Tracing.catching(EditingDomainManager.class, "createEditingDomain",
        // e); //$NON-NLS-1$
     ...
     * @param id the ID of an editing domain
     * @throws IllegalArgumentException if the specified ID is statically
     *         registered
     * @since 1.3
     */
    public void assertDynamicallyRegistered(String id)
    {
        if (isStaticallyRegistered(id)) {
            IllegalArgumentException exc =
                    new IllegalArgumentException("NLS.bind(Messages.removeStaticDomain, id)");
            Tracing.throwing(EditingDomainManager.class, "assertDynamicallyRegistered", exc); //$NON-NLS-1$
            throw exc;
        }
    }

    /**
     * Creates (if necessary) listeners registered against the specified editing
     * domain ID and and adds them to it. Note that this includes listeners
     * registered against all domains (by not specifying an ID).
     * <p>
     ...
     * its ID. Note that this includes listeners registered against all domains
     * (by not specifying an ID).
     * @param id the editing domain ID
    ...
    /**
     * Retrieves the configuration element for the extension providing the
     * specified domain ID.
     * @param id the domain ID to retrieve
     * @return the corresponding configuration element, or <code>null</code> if
     *         this ID is not registered on the extension point
     */
    ...
    // if (E_DOMAIN.equals(configs[i].getName()) &&
    // id.equals(configs[i].getAttribute(A_ID))) {
    // result = configs[i];
    // }
    // }
    //
    ...
    /**
     * Retrieves the configuration elements for listeners registered on the
     * specified editing domain ID. This includes listeners that are registered
     * against all editing domains.
     * @param id the domain ID to retrieve
    ...
    // {
    // Collection<IConfigurationElement> result = new
    // java.util.ArrayList<IConfigurationElement>();
    //
    // IConfigurationElement[] configs =
    // Platform.getExtensionRegistry()
    // .getConfigurationElementsFor(EMFTransactionPlugin.getPluginId(),
    ...
    /**
     * Retrieves the configuration elements for listeners that are registered on
     * all editing domains.
     ...
     *         invalid or <code>create = false</code> and the listener does not
     *         yet exist
     */
    // private ResourceSetListener getListener(IConfigurationElement config,
    // boolean create)
    // {
    // ResourceSetListener result = null;
    //
    // Reference<ResourceSetListener> ref =
    // listeners.get(config.getAttribute(A_CLASS));
    // if (ref != null) {
    // result = ref.get();
    // }
    //
    ...
    // } else {
    // EMFTransactionPlugin.getPlugin()
    // .log(new Status(IStatus.ERROR,
    // EMFTransactionPlugin.getPluginId(),
    // EMFTransactionStatusCodes.LISTENER_TYPE,
    // NLS.bind(Messages.listenerInterface,
    ...
    // //$NON-NLS-1$
    // EMFTransactionPlugin.INSTANCE.log(new
    // MultiStatus(EMFTransactionPlugin.getPluginId(),
    // EMFTransactionStatusCodes.LISTENER_INITIALIZATION,
    // new IStatus[]{e.getStatus()},
    // NLS.bind(Messages.listenerInitialization,
    // config.getAttribute(A_CLASS)),
    // null));
    // }
    // }
    //
