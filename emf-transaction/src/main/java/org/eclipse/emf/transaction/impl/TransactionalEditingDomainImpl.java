/**
 * <copyright> Copyright (c) 2005, 2008 IBM Corporation, Zeligsoft Inc., and
 * others. All rights reserved. This program and the accompanying materials are
 * made available under the terms of the Eclipse Public License v1.0 which
 * accompanies this distribution, and is available at
 * http://www.eclipse.org/legal/epl-v10.html Contributors: IBM - Initial API and
 * implementation Zeligsoft - Bugs 177642, 145877, 245446 </copyright> $Id:
 * TransactionalEditingDomainImpl.java,v 1.22 2010/01/07 15:29:51 bgruschko Exp
 * $
 */
 ...
 * <p>
 * Since 1.2, this class implements the {@link Adaptable} interface to adapt to
 * the following optional API:
 * </p>
 * <ul>
 ...
 */
public class TransactionalEditingDomainImpl extends AdapterFactoryEditingDomain implements
        InternalTransactionalEditingDomain, Adaptable, TransactionalEditingDomain.DefaultOptions
{
    ...
    private final Map<Object, Object> defaultTransactionOptionsRO =
            Collections.unmodifiableMap(defaultTransactionOptions);
    ...
    private final Map<Object, Object> undoRedoOptions =
            new java.util.HashMap<Object, Object>(TransactionImpl.DEFAULT_UNDO_REDO_OPTIONS);
    ...
    /**
     * Initializes me with my adapter factory and a command stack, and a default
    ...
    /**
     * Initializes me with my adapter factory and default implementations of a
     * resource set and a command stack.
     * @param adapterFactory my adapter factory
     */
    public TransactionalEditingDomainImpl(AdapterFactory adapterFactory)
    {
    ...
    private void initialize()
    {
        ...
        resourceToReadOnlyMap = new HashMap<>();// java.util.WeakHashMap<Resource,
                                                // Boolean>();
    }
    ...
    public void addResourceSetListener(ResourceSetListener l)
    {
        if (l.isPrecommitOnly() && l.isPostcommitOnly()) {
            throw new IllegalArgumentException("conflicting isPrecommitOnly() and isPostcommitOnly()"); //$NON-NLS-1$
        }
        ...
        synchronized (precommitListeners) {
            synchronized (aggregatePrecommitListeners) {
                synchronized (postcommitListeners) {
                    ...
                    // add the listener to the appropriate list only if it
                    // expects
                    // to receive the event type and is not already in the list

                    if (!l.isPostcommitOnly()) {
                        if (!l.isAggregatePrecommitListener() && !precommitListeners.contains(l)) {
        ...
        synchronized (precommitListeners) {
            synchronized (aggregatePrecommitListeners) {
                synchronized (postcommitListeners) {
                    ...
                    if (wasRemoved && (l instanceof ResourceSetListener.Internal)) {
    ...
    public Object runExclusive(Runnable read) throws InterruptedException
    {
        ...
        if ((active == null) || !(active.isActive() && active.isReadOnly()))
        ...
        final RunnableWithResult<?> rwr =
                (read instanceof RunnableWithResult) ? (RunnableWithResult<?>) read : null;
        ...
        try {
            ...
            if ((tx != null) && (tx.isActive())) {
                ...
                try {
                    ...
                    EMFTransactionPlugin.INSTANCE.log(new MultiStatus(EMFTransactionPlugin.getPluginId(),
    ...
    public void yield()
    {
        ...
        // new IllegalStateException("Only the active transaction may yield");
        // //$NON-NLS-1$
        ...
        // Tracing.trace(">>> Yielding " //$NON-NLS-1$
        // + getDebugID(activeTransaction)
        // + " at " //$NON-NLS-1$
        // + Tracing.now());
        // }
        //
        ...
        //
        // if (Tracing.shouldTrace(EMFTransactionDebugOptions.TRANSACTIONS)) {
        // Tracing.trace(">>> Resuming " //$NON-NLS-1$
        // + getDebugID(activeTransaction)
        // + " at " //$NON-NLS-1$
        // + Tracing.now());
        // }
    ...
    {
        return getDebugID(tx.getEditingDomain())
               + "::" //$NON-NLS-1$
               + ((tx instanceof TransactionImpl)
                       ? Long.toString(((TransactionImpl) tx).id)
                       : "<anonymous>"); //$NON-NLS-1$
    }
    ...
    {
        synchronized (precommitListeners) {
            return precommitListeners.toArray(new ResourceSetListener[precommitListeners.size()]);
        }
    }
     ...
     * @return my aggregate pre-commit listeners (as of the time of calling this
     *         method)
     */
    protected final ResourceSetListener[] getAggregatePrecommitListeners()
    {
        synchronized (aggregatePrecommitListeners) {
            return aggregatePrecommitListeners.toArray(new ResourceSetListener[aggregatePrecommitListeners.size()]);
        }
    }
    ...
    {
        synchronized (postcommitListeners) {
            return postcommitListeners.toArray(new ResourceSetListener[postcommitListeners.size()]);
        }
    }
    ...
    public void deactivate(InternalTransaction tx)
    {
        ...
        if (activeTransaction != tx) {
            IllegalArgumentException exc =
                    new IllegalArgumentException("Can only deactivate the active transaction"); //$NON-NLS-1$
        ...
        try {
            if (activeTransaction == null) {
                // deactivation of a root transaction generates post-commit
                // event
                postcommit(tx);
                ...
                // ensure that the validator no longer retains this transaction
                // in
    ...
    private void acquire(InternalTransaction tx) throws InterruptedException
    {
            ...
            && !TransactionImpl.isUnprotected(tx))
        {
            ...
            throw new IllegalStateException("Cannot activate read/write transaction in read-only transaction context"); //$NON-NLS-1$
        }
    ...
    public void precommit(final InternalTransaction tx) throws RollbackException
    {
        class PrecommitRunnable extends RunnableWithResult.Impl<List<Command>>
        {
            ...
            List<Command> runExclusive() throws InterruptedException
            {
                if ((listeners.length > 0) && !notifications.isEmpty()) {
                    return TransactionUtil.runExclusive(TransactionalEditingDomainImpl.this, this);
                } else {
            ...
            public void run()
            {
                ...
                ArrayList<Notification> cache = new ArrayList<Notification>(notifications.size());

                for (ResourceSetListener element : listeners) {
                    try {
                        List<Notification> filtered =
                                FilterManager.getInstance()
                                             .select(notifications, element.getFilter(), cache);

                        if (!filtered.isEmpty()) {
                            Command cmd =
                                    element.transactionAboutToCommit(new ResourceSetChangeEvent(TransactionalEditingDomainImpl.this,
                                                                                                tx,
                                                                                                filtered));
                        ...
                        Tracing.catching(TransactionalEditingDomainImpl.class, "precommit", e); //$NON-NLS-1$
                        IStatus status =
                                new Status(IStatus.ERROR, EMFTransactionPlugin.getPluginId(),
                                           EMFTransactionStatusCodes.PRECOMMIT_FAILED,
                                           "Messages.precommitFailed", e);
        ...
        PrecommitRunnable runnable =
                new PrecommitRunnable(getPrecommitListeners(), tx.getNotifications());
        ...
        while (runnable != null) {
            try {
                ...
                if (!triggers.isEmpty()) {
                    getTransactionalCommandStack().executeTriggers(command,
                                                                   triggers,
                                                                   tx.getOptions());
                }

                List<Notification> notifications = validator.getNotificationsForPrecommit(tx);

                if ((notifications == null) || notifications.isEmpty()) {
                    runnable = null;
                } else {
                    runnable =
                            new PrecommitRunnable(getAggregatePrecommitListeners(), notifications);
                }
            }
            catch (InterruptedException e) {
                Tracing.catching(TransactionalEditingDomainImpl.class, "precommit", e); //$NON-NLS-1$
                IStatus status = new Status(IStatus.ERROR, EMFTransactionPlugin.getPluginId(),
    ...
    protected void postcommit(final InternalTransaction tx)
    {
        ...
        final List<Notification> notifications = validator.getNotificationsForPostcommit(tx);
        if ((notifications == null) || notifications.isEmpty()) {
            return;
        }
        final ArrayList<Notification> cache = new ArrayList<Notification>(notifications.size());
        ...
        try {
                ...
                public void run()
                {
                    for (ResourceSetListener element : listeners) {
                        try {
                            ...
                            if (!filtered.isEmpty()) {
                                element.resourceSetChanged(new ResourceSetChangeEvent(TransactionalEditingDomainImpl.this,
                                                                                      tx,
                                                                                      filtered));
                            }
                        }
            ...
            IStatus status = new Status(IStatus.ERROR, EMFTransactionPlugin.getPluginId(),
                                        ...
                                        "Messages.postcommitInterrupted", e);
            EMFTransactionPlugin.INSTANCE.log(status);
        }
    }
                ...
                public void run()
                {
                    for (ResourceSetListener element : listeners) {
                        try {
                            List<Notification> filtered =
                                    FilterManager.getInstance()
                                                 .selectUnbatched(unbatchedNotifications,
                                                                  element.getFilter());
    ...
    public final RunnableWithResult<Object> createPrivilegedRunnable(Runnable runnable)
    {
        ...
        if ((tx == null)) {// || (tx.getOwner() != Thread.currentThread())) {
            throw new IllegalStateException("active transaction not owned by caller"); //$NON-NLS-1$
        }
    ...
    public void startPrivileged(PrivilegedRunnable<?> runnable)
    {
        if (runnable.getTransaction().getEditingDomain() != this) {
            throw new IllegalArgumentException("runnable has no privileges on this editing domain"); //$NON-NLS-1$
        }
    ...
    public void dispose()
    {
        if (!disposed) {
            ...
            getLifecycle().fireLifecycleEvent(TransactionalEditingDomainEvent.EDITING_DOMAIN_DISPOSING,
                                              null);
            ...
            // clear listeners after notification so that they cannot add
            // themselves
    ...
    /**
     * Obtains the factory that this transactional editing domain uses to create
     * validators for transaction validation.
     * <p>
    ...
    /**
     * Creates a new lifecycle implementation. Subclasses may override to create
    ...
    /**
     * Creates a new transaction option metadata registry. Subclasses may
     * override to create their own implementation, although it would hardly
     * seem interesting to do so.
     ...
     * class creates {@link TransactionalEditingDomainImpl}s and provides the
     * mapping of resource sets to editing domain instances.
     * <p>
     * Clients that implement their own factory can plug in to the mapping of
     * resource sets to editing domains using the static instance's
     * {@link #mapResourceSet(TransactionalEditingDomain)} and
     * {@link #unmapResourceSet(TransactionalEditingDomain)} methods by casting
     * the {@link TransactionalEditingDomain.Factory#INSTANCE} to the
    ...
    public static class FactoryImpl implements TransactionalEditingDomain.Factory
    {
        ...
        public synchronized TransactionalEditingDomain createEditingDomain()
        {
            TransactionalEditingDomain result =
                    new TransactionalEditingDomainImpl(new ComposedAdapterFactory(ComposedAdapterFactory.Descriptor.Registry.INSTANCE));
        ...
        {
            // for (Iterator<Adapter> iter =
            // domain.getResourceSet().eAdapters().iterator(); iter.hasNext();)
            // {
            //
        ...
        /**
         * An adapter that attaches a weak reference to the editing domain onto
         * the resource set that it manages.
         * @author Christian W. Damus (cdamus)
         */
    ...
    public final static class RegistryImpl implements TransactionalEditingDomain.Registry
    {
         ...
         * method must only be invoked after it has been determined that this ID
         * can be registered.
    ...
    /**
     * Default implementation of the {@link InternalLifecycle} protocol. May be
     * subclassed by custom editing domain implementations.
     * @author Christian W. Damus (cdamus)
     * @since 1.3
     */
    protected final class LifecycleImpl implements InternalLifecycle
    {

        private final List<TransactionalEditingDomainListener> lifecycleListeners =
                new java.util.ArrayList<TransactionalEditingDomainListener>();
        ...
        {
            synchronized (lifecycleListeners) {
                return lifecycleListeners.toArray(new TransactionalEditingDomainListener[lifecycleListeners.size()]);
            }
        }

        /**
         * Fires the specified life-cycle event to my listeners, if any.
         * @param type one of the life-cycle event
         *        {@linkplain TransactionalEditingDomainEvent#TRANSACTION_STARTING
         *        types}
         * @param transaction the transaction that is the subject of the event,
         *        or <code>null</code> if the event pertains to the editing
         *        domain, itself
         */
        protected void fireLifecycleEvent(int type, Transaction transaction)
        {
            ...
            TransactionalEditingDomainEvent event =
                    new TransactionalEditingDomainEvent(TransactionalEditingDomainImpl.this, type,
                                                        transaction);
            ...
            for (TransactionalEditingDomainListener next : getLifecycleListeners()) {
                try {
                    ...
                    Tracing.catching(TransactionalEditingDomainImpl.class, "fireLifecycleEvent", e); //$NON-NLS-1$
        ...
        {
            fireLifecycleEvent(TransactionalEditingDomainEvent.TRANSACTION_CLOSED, transaction);
        }

        @Override
        public void transactionClosing(InternalTransaction transaction)
        {
            fireLifecycleEvent(TransactionalEditingDomainEvent.TRANSACTION_CLOSING, transaction);
        }

        @Override
        public void transactionInterrupted(InternalTransaction transaction)
        {
            fireLifecycleEvent(TransactionalEditingDomainEvent.TRANSACTION_INTERRUPTED,
                               transaction);
        }
        ...
        {
            fireLifecycleEvent(TransactionalEditingDomainEvent.TRANSACTION_STARTED, transaction);
        }

        @Override
        public void transactionStarting(InternalTransaction transaction)
        {
            fireLifecycleEvent(TransactionalEditingDomainEvent.TRANSACTION_STARTING, transaction);
        }
