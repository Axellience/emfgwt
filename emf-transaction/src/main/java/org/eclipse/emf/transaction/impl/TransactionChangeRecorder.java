 ...
 * implementation Zeligsoft - Bug 250498 </copyright> $Id:
 * TransactionChangeRecorder.java,v 1.8 2008/11/16 13:23:38 cdamus Exp $
 */
 ...
 * {@link org.eclipse.emf.transaction.TransactionalEditingDomain}, used by
 * transactions to record rollback information and to detect changes that
...
public class TransactionChangeRecorder extends ChangeRecorder
{
    ...
    /**
     * Initializes me with the editing domain that I assist and the resource set
     * in which I will record changes. Note that I do not begin recording until
     * specifically asked to (unlike the superclass).
    ...
    /**
     * Extends the inherited implementation to clear the reference to the change
     ...
     * <ul>
     * <li>ignore the "originalTargetObjects" since we never resume recording a
     * paused change description</li>
     * <li>ignore the "targetObjects" because we will never find, upon upon
     * consolidating changes, that any target object is unexpectedly orphaned
     * (as we always listen to everything in the resource set, so will always
     * get the appropriate removal notifications). Also, because we manage an
     * entire resource set on behalf of an editing domain, disposal by removing
     * ourselves from the adapters lists of our targets is not an issue because
     * we cannot cause a memory leak outside of the scope of the editing domain
     * and its resource set</li>
     * </ul>
     */
    @Override
            ...
            case Notification.MOVE:
                ...
                if (sourceRes != null) {
                    ...
                    if (!notification.isTouch() && (getValidateEditSupport() != null)) {
                        getValidateEditSupport().handleResourceChange(sourceRes, notification);
                    }
                }
     ...
     * resources and violations of the transaction protocol before passing it to
    ...
    {
        ResourceSetManager.getInstance()
                          .observe((ResourceSet) notification.getNotifier(), notification);

        appendNotification(notification);
    }

    /**
     * Analyzes a resource notification for changes in its load state and
     * violations of the transaction protocol before passing it to the active
     * transaction (if any).
    ...
    {
        ResourceSetManager.getInstance()
                          .observe((Resource) notification.getNotifier(), notification);
     ...
     * transaction, to be distributed when it commits. If there is no active
     * transaction, then it is sent immediately to post-commit listeners
     * (unbatched). This method applies the read/write transaction protocol
     * check to this notification.
    ...
    protected void appendNotification(Notification notification)
    {
        ...
        if (tx != null) {
            ...
            tx.add(notification);
            // } else if
            // (Tracing.shouldTrace(EMFTransactionDebugOptions.TRANSACTIONS)) {
            //
            // Tracing.trace("Read notification from thread " //$NON-NLS-1$
            // + Thread.currentThread().getName()
            // + " not appended to " //$NON-NLS-1$
            // + tx);
            // }
        } else {
     ...
     * @throws IllegalStateException if no transaction is active or the active
    ...
    protected void assertWriting()
    {
        ...
        if ((tx == null) || tx.isReadOnly()) { // || (tx.getOwner() !=
            ...
            synchronized (domain) {
                ...
                if (tx != null) {
                    tx.abort(new Status(IStatus.ERROR, EMFTransactionPlugin.getPluginId(),
    ...
    public void dispose()
    {
        if (!disposed) {
            ...
            if (rset != null) {
                ...
                for (Iterator<Notifier> iter =
                        EcoreUtil.getAllProperContents(rset, false); iter.hasNext();)
                {
    ...
    /**
     * Sets my validate-edit support, if applicable, for the duration of a root
     * transaction.
