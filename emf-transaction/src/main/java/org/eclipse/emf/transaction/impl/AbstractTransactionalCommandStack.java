/**
 * <copyright> Copyright (c) 2007, 2013 IBM Corporation and others. All rights
 * reserved. This program and the accompanying materials are made available
 * under the terms of the Eclipse Public License v1.0 which accompanies this
 * distribution, and is available at http://www.eclipse.org/legal/epl-v10.html
 * Contributors: IBM - Initial API and implementation </copyright> $Id:
 * AbstractTransactionalCommandStack.java,v 1.5 2007/11/14 18:14:00 cdamus Exp $
 */
 ...
 * @since 1.1
 */
public abstract class AbstractTransactionalCommandStack extends BasicCommandStack
        implements InternalTransactionalCommandStack
{
    ...
    /**
     * Ensures that the specified transaction is rolled back, first rolling back
     * a nested transaction (if any).
    ...
    /**
     * Extends the inherited method by first aborting the active transaction (if
     * any) and passing the exception along to the registered exception handler
     * (if any).
     */
    @Override
    protected void handleError(Exception exception)
    {
        ...
        if ((active != null) && active.isActive()) {
            active.abort(new Status(IStatus.ERROR, EMFTransactionPlugin.getPluginId(),
     ...
     *         or a {@link RollbackException} that was caused by operation
     *         cancel
     */
     ...
     * command is rolled back. Note that <code>doExecute()</code> is only called
     * if the command is {@linkplain Command#canExecute() executable}.
     */
    @Override
     ...
     * @param options the transaction options to apply to execution of the
     *        command
    ...
    /**
     * Customizes the specified <code>options</code> for the case of a
     * transaction that executes trigger commands. The original map is not
     * affected.
    ...
    public static final Map<Object, Object> makeTriggerTransactionOptions(Map<?, ?> options)
    {
        ...
        if ((options == null) || options.isEmpty()) {
            result = Collections.<Object, Object>singletonMap(TransactionImpl.OPTION_IS_TRIGGER_TRANSACTION,
                                                              Boolean.TRUE);
        } else {
            result = new java.util.HashMap<Object, Object>(options);
            result.put(TransactionImpl.OPTION_IS_TRIGGER_TRANSACTION, Boolean.TRUE);
        }
