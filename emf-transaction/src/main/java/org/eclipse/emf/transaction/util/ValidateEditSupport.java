/**
 * <copyright> Copyright (c) 2007 IBM Corporation and others. All rights
 * reserved. This program and the accompanying materials are made available
 * under the terms of the Eclipse Public License v1.0 which accompanies this
 * distribution, and is available at http://www.eclipse.org/legal/epl-v10.html
 * Contributors: IBM - Initial API and implementation </copyright> $Id:
 * ValidateEditSupport.java,v 1.2 2007/11/14 18:14:00 cdamus Exp $
 */
 ...
 * can be made to be editable. The root-level transaction should assign this to
 * the {@link TransactionChangeRecorder} when it starts and remove it when it
 * closes.
 * </p>
 * <p>
...
public interface ValidateEditSupport
{
     ...
     * @param transaction the root-level transaction that is attempting to
     *        commit
     * @param context usually a <code>org.eclipse.swt.widgets.Shell</code>
     *        providing a UI context for interaction with the user to make
     *        resources modifiable, or <code>null</code> if no such context is
     *        available and the system should attempt to automatically validate
     * @return a status indicating <tt>OK</tt> if validate-edit succeeded,
     *         otherwise some reason why it failed and the transaction should
     *         roll back
     */
     ...
     * either indicate that the resource should be added to the validate list
     * (i.e., its persisted state is changed) or it should be removed from the
     * validate list (e.g., because it has been unloaded).
    ...
    class Default implements ValidateEditSupport
    {
         ...
         * @param context the validate-edit context, or <code>null</code> if
         *        none
        ...
        public void finalizeForRollback()
        {
            if (!resourcesToValidate.isEmpty()) {
                ...
                Resource[] resources =
                        resourcesToValidate.toArray(new Resource[resourcesToValidate.size()]);
